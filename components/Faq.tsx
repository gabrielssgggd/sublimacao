      ...
      <div className="container mx-auto px-4 max-w-3xl">
        <div className="flex flex-col items-center mb-16">
            <div className="p-3 bg-white/5 rounded-full mb-6">
                <HelpCircle className="text-gold-400 w-8 h-8" />
            </div>
            <h2 className="text-3xl md:text-4xl font-display font-bold text-center text-white">Dúvidas <span className="text-gold-400">Frequentes</span></h2>
        </div>
        
        <div className="space-y-4">
          {faqData.map((item, index) => (
            <div key={index} className={`border rounded-xl bg-dark-900 overflow-hidden transition-all duration-300 ${activeIndex === index ? 'border-gold-500/30 shadow-glow bg-dark-850' : 'border-white/5 hover:border-white/10'}`}>
              <button 
                onClick={() => setActiveIndex(activeIndex === index ? null : index)}
                className="w-full p-6 flex items-center justify-between text-left focus:outline-none"
              >
                <span className={`font-semibold text-lg transition-colors ${activeIndex === index ? 'text-white' : 'text-gray-300'}`}>{item.question}</span>
                <div className={`p-1 rounded-full transition-all ${activeIndex === index ? 'bg-gold-500 text-black rotate-180' : 'bg-white/5 text-gray-400'}`}>
